...
const apiConfig = require('./webpack.api.config.js');
...
const { hasProjectYarn, openBrowser } = require('./lib');
...
module.exports = (app, cb) => {
      ...
      if (isFrist) {
        ...
        console.log(
          chalk.bgGreen.black(' DONE ') +
            ' ' +
            chalk.green(
              `Compiled successfully in ${serverTime + webTime + apiTime}ms`,
            ),
        );
        console.log();
        console.log(`  App running at: ${chalk.cyan(url)}`);
        console.log();
        const buildCommand = hasProjectYarn(process.cwd())
          ? `yarn build`
          : `npm run build`;
  ...
  apiCompiler.watch({}, (err, stats) => {
    ...
    apiMain = apiMfs.readFileSync(
      path.join(apiConfig.output.path, 'api.js'),
      'utf-8',
    );
    update();
  });
